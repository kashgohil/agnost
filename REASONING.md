...
**Why:** a topical cluster contains multiple *patterns* — same topic, different stories. Collapsing them into one insight averages the metrics across mixed populations and inflates the volume. Real example from the build: the refund cluster initially showed 183 conversations and -0.38 sentiment, but only 110 actually failed at the tool — the other 73 were satisfied users and silent drop-offs that the headline didn't speak for. After partitioning: `failed_at_tool` shows 133 conversations at -0.57 sentiment (much closer to the seeded 110, with the brief example shape recovered), and `succeeded` surfaces separately as its own positive-pattern insight.
...
Failure attribution (the brief's "due to X") is literally this query:
...
The `DISTINCT` CTE is load-bearing — without it, N user turns from one conversation multiply tool-call counts by N. (Caught and fixed mid-build.) **Switch point** to a dedicated vector store: 10M+ vectors with embedding-dominated workload. Not close.
...
**Why:** provider flexibility for free. Single billing surface. I had ~$100 in credits there.
