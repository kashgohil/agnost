// Aggregates per (cluster, partition) metrics. See REASONING.md §2 for why
// (cluster, partition) is the insight unit, and proximity.ts for how tool
// calls are attributed across clusters.
...
export async function aggregateAllClusters(): Promise<ClusterMetrics[]> {
  ...
  const allSignals = await db
    .select({
      turnId: schema.turnSignals.turnId,
  ...
  const precedingUserMap = buildPrecedingUserTurnMap(allTurns);
  const turnIntentMap = new Map(allSignals.map((s) => [s.turnId, s.intent]));
  const intentClusterMap = new Map(allIntents.map((i) => [i.intent, i.clusterId]));
  ...
  const turnConversation = new Map(allTurns.map((t) => [t.id, t.conversationId]));
  ...
  const attributedCalls: AttributedToolCall[] = [];
  for (const tc of toolCallRows) {
    const cluster = turnClusterMap.get(tc.turnId);
    if (!cluster) continue;
    const conversationId = turnConversation.get(tc.turnId);
  ...
  for (const c of attributedCalls) {
    const k = `${c.clusterId}::${c.conversationId}`;
    const f = facts.get(k);
    if (!f) continue;
    f.toolCallCount += 1;
    f.toolLatencySum += c.latencyMs;
    if (isFailedToolStatus(c.status)) f.anyToolFailed = true;
  }
  ...
  type Acc = {
    ...
    isRepeatSum: number;
    turnSum: number;
    weeklyVolume: Map<string, Set<string>>;
  ...
  const conversationPartition = new Map<string, OutcomePartition>();
  for (const f of facts.values()) {
    conversationPartition.set(`${f.clusterId}::${f.conversationId}`, f.partition);
  }
  type ToolAcc = { calls: number; failed: number };
  const toolAccs = new Map<Key, Map<string, ToolAcc>>();
  for (const c of attributedCalls) {
    const partition = conversationPartition.get(`${c.clusterId}::${c.conversationId}`);
    if (!partition) continue;
    const k = key(c.clusterId, partition);
    let byTool = toolAccs.get(k);
    if (!byTool) {
      byTool = new Map();
      toolAccs.set(k, byTool);
    }
    const ta = byTool.get(c.toolName) ?? { calls: 0, failed: 0 };
    ta.calls += 1;
    if (isFailedToolStatus(c.status)) ta.failed += 1;
    byTool.set(c.toolName, ta);
  }
  ...
  for (const [k, a] of accs) {
    ...
    // Suppress attribution on positive-sentiment partitions — happy clusters
    // with incidentally-failing tools are coincidence, not causation.
    const attributedCause = sentimentAvg >= 0 ? null : (attribCause.get(k) ?? null);
